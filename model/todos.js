  ...
  module.exports=class Todos{
      ...
      static getTodos(user){
          return db.collection('todos').then((e)=>{
              return e.aggregate([
                  {$match:{userId:user._id}},
                  {"$group":{_id:{list:'$list'}, count:{$sum:2}, todo: {$push: { name: '$name', desc: '$desc',finished:'$finished', expire: { $dateToString: { format: "%Y/%m/%d %H:%M:%S", date: "$expire" } },id:"$_id" }}}} , 
      ...
      static day(day,user){
                  ...
                  {$match:{userId:user._id}},
                   {
                       ...
                       todo: {$push: { name: '$name', desc: '$desc',finished:'$finished', expire: { $dateToString: { format: "%Y/%m/%d %H:%M:%S", date: "$expire" } },id:"$_id" }}
                      } 
                   },
                   {$match:{"_id.day":day}}
                ]
        ...
        static groupByFinished(finish,user){
          ...
          return  e.aggregate(
                [
                  {$match:{userId:user._id,finished:finish}},
                   {
                       $group:{_id:{
        ...
        static expire(user,date,current){
          return db.collection('todos').then((e)=>{
              return e.aggregate([
                  ...
                  {$match:{userId:user._id}},
                  {"$group":{_id:{list:'$list'}, count:{$sum:1}, todo: {$push: { name: '$name', desc: '$desc',finished:'$finished', expire: { $dateToString: { format: "%Y/%m/%d %H:%M:%S", date: "$expire" } },id:"$_id" }}}} , 
